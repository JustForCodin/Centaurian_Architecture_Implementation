...
This motivates a directional shift, pursued from Experiment 6 onward: rather than steering one large frozen transducer, **ADA is built as a set of small, per-scenario language models that are trained and fully owned** — with any QPM/persona conditioning installed at the *weight* level (the mechanism that already succeeded for persona in Experiment 2, d = 7.51) rather than injected at inference. The driving requirements are **offline resilience and full local ownership**: a self-contained `(corpus, weights)` artifact that needs no network to run or to retrain. The first test of this direction — **Experiment 6 — validates it.** It trains a model **from scratch** (no pretrained base) as ADA's *daily-QA* specialist — the cheapest scenario, since its facts come from retrieval (Section 6), so the model need only read context, answer or abstain, and stay in character. An initial ~80M model localised answers poorly by generation; the design that worked is a **dual-head trunk** at ~321M (BERT-large scale): a bidirectional **span + answerability head** for extractive reading/abstention, and a causal **LM head** that carries ADA's voice and hosts the persona, with the QPM compiled into the weights via a `<|persona|>` channel rather than fed to a frozen model at inference.

Experiment 6 cleared its operational bars — grounded reading **H1 = 0.77**, calibrated abstention **H2 = 0.78** (both past the 0.70 deployment bar), and persona **H3 = 3.80** across 40 turns (≥ 3.5, no turn-based degradation) — and produced three results that bear directly on this architecture's thesis (full record in Section 15.2.5). First, **weight-level installation resolves the Experiment 3–5 interface-null**: compiling the QPM/persona into the weights lets the signal shape behaviour as *supervision*, whereas every inference-time channel discarded it (the QPM-on/off ablation is a marginal positive at weak baking, neutral at strong baking, never harmful). Second, **baking beats prompting across a 23× size gap**: the 321M owned model outscores a *prompted* 7B on every PersonaScore dimension (3.80 vs 3.22) and reaches parity with the *fine-tuned* 7B on the substantive dimensions — Capability identical (4.47 vs 4.47), Episodic tied (3.29 vs 3.35, and from weights alone versus the 7B's episodic RAG) — trailing only on Trait/Style, where raw generative scale dominates. Third, **a weight-baked persona needs no SCI refresh** (Section 17.6): re-injecting the SCI at turns 15/30 did not help (R1 3.75 ≈ R0 3.80), contradicting Experiment 1's frozen-model refresh-necessity. The owned 321M `(corpus, weights)` artifact runs fully offline (int8, ~150–300 MB; ~50–100 tok/s on a Jetson Orin Nano), realising the offline-resilience requirement. H2's residual gap to the aspirational 0.80 is a pretraining-*objective* limit — a causal trunk retrofitted bidirectionally versus a native MLM encoder — not a scale limit (the model is already BERT-large-sized), and keeping the trunk causal is what lets the *same* model *be* ADA rather than only read.

**Experiment 7 extends the owned agent from a 40-turn QA specialist to a long-running daily agent** (Section 15.2.6). Because the agent is stateless per-turn (its SCI overflows the context window), long-horizon continuity is supplied by an **external episodic-RAG memory layer** rather than by scale. The load-bearing finding is a role split: **episodic factual recall must run on the discriminative reader (retrieve → span-extract), not the generative head** — the generative head fabricates values, while the span head recalls reliably and lag-independently (0.81 with perfect storage, ≥ 0.70 at every distance). Persona and the O(1)-cost memory scaffolding stay on the owned small model; only the peak write-accuracy path (storing facts cleanly at 321M) points to the larger-base fallback. Memory-aware fine-tuning of the generative head to close this is a documented dead end at this scale — the resolution is architectural, not more data.
...
This section specifies the evaluation methodology for the Centaurian Architecture. **Experiments 1, 2, 6, and 7 (Behavioral Consistency — SCI/SMC component)** are complete, along with the QPM-interface ablations 3–5 (summarised in Section 5.9). Experiment 1 established the 7B baseline and tested six prompt-time architectural strategies; Experiment 2 added LoRA fine-tuning under a 4-condition design with H5 data-scaling sub-runs, achieving Outcome A under the pre-registered decision rule and validating the full Phase 1 SMC architecture (Section 17.5); Experiment 6 then executed the Section 5.9 direction, validating a **from-scratch, fully-owned 321M daily-QA agent** with the persona/QPM compiled into the weights (Section 15.2.5). See Section 15.2 for high-level findings and Sections 15.4.1, 15.4.2 for detailed protocols. Experiment 7 extended the owned agent to long-horizon episodic memory (Section 15.2.6). The remaining evaluation dimensions (Speech Naturalness, Uncanny Valley) are deferred to future work. The validation framework is designed to be executable with Phase 1 components (Section 16.2).
...
| **Behavioral Consistency** | Stability of personality across interactions | AI researchers, domain deployers | Completed (Experiments 1, 2, and 6; incl. the H4 base-capability test: +1.71% mean degradation, p = 0.287 — Section 15.4.2 — and the from-scratch owned agent, Section 15.2.5) |
| **Persona Coherence Over Time** | Whether the agent presents as a consistent, recognizable persona across long and multi-session interactions (rather than whether it can be mistaken for human) | Human-computer interaction researchers | Completed (Experiment 7 — persona holds ~3.79 over 100–500 turns; episodic recall via extract-then-style; Section 15.2.6) |
...
### 15.2 Completed: SCI/Persona Experiments (1, 2) and the From-Scratch Owned Agent (6)
...
3. **Does the Combined intervention's late-conversation score improvement generalize across domains beyond psychotherapy?** The unique pattern of scores being higher at turns 25–40 than turns 5–10 requires replication on a non-therapy persona to confirm robustness. **Partially addressed:** Experiment 6 evaluated a non-therapy persona (the ADA daily-QA discovery assistant) and held PersonaScore at 3.80 across 40 turns — but with a *flat* per-turn curve rather than the late-conversation uplift, because the from-scratch model is effectively stateless per-turn (Section 15.2.5). The specific rising-curve pattern therefore remains open for a stateful long-horizon setting (Experiment 7).
...
#### 15.2.5 Experiment 6: From-Scratch Owned Daily-QA Agent (validating Section 5.9)

Experiment 6 executed the Section 5.9 direction: a from-scratch, fully-owned small model as ADA's daily-QA specialist, with persona/QPM compiled into the weights. **Architecture:** a 321M-parameter (BERT-large-scale) Llama-style decoder with a **dual-head trunk** — a bidirectional **span + answerability head** for extractive reading/abstention, and a causal **LM head** carrying ADA's voice and a `<|persona|>` QPM channel. Trained on free corpora (FineWeb-Edu pretrain; SQuAD 2.0 reading/abstention supervision; OASST2 + Dolly instruction substrate) plus a bounded Sonnet-generated ADA persona layer. Same PersonaScore harness and Sonnet 4.5 judge as Experiments 1–3.

**Key findings:**

11. **The owned-small-model direction is validated on its first scenario.** On the deployment bars the model is ✓✓✓: grounded reading **H1 = 0.77** (SQuAD2 F1 0.768 / EM 0.695), calibrated abstention **H2 = 0.78** (both past 0.70), and persona **H3 = 3.80/5.0** across 40 turns (≥ 3.5), with a flat per-turn curve — no degradation inflection (T\* = None). Judge reliability κ_w = 0.99.

12. **Weight-level installation resolves the Experiment 3–5 interface-null.** Compiling the QPM/persona into the weights lets the signal shape behaviour as *supervision*, where every inference-time channel (JSON marginals, JSON enrichment, activation steering) discarded it. The QPM-on/off ablation (RQ6) is a marginal positive at weak baking (+0.09 at overall 3.1) and neutral at strong baking (3.80 vs 3.79) — never harmful. The coherence-as-*distinguishability* question (Experiment 5's p = 0.72 null) remains owed to the therapy scenario, where coherence is the payload.

13. **Baking beats prompting across a 23× size gap.** The 321M owned model outscores a *prompted* Qwen2.5-7B on every PersonaScore dimension (3.80 vs 3.22) and reaches parity with the *LoRA-fine-tuned* 7B on the substantive dimensions — Capability identical (4.47 vs 4.47), Episodic tied (3.29 vs 3.35, and *from weights alone* versus the 7B's episodic RAG) — trailing only on Trait (3.73 vs 4.90) and Style (3.69 vs 4.94), the free-form stylistic-expression dimensions where raw generative scale dominates. **The small-vs-large gap is stylistic fluency, not persona substance.** (Cross-experiment caveat: Experiments 1–3 use the Aria therapy persona, Experiment 6 the terser ADA daily-QA persona; harness and judge are identical, so the dimensions measure the same constructs.)

14. **A weight-baked persona needs no SCI refresh (H4 = refresh-unnecessary).** Re-injecting the SCI at turns 15/30 did not help (R1 3.75 ≈ R0 3.80, |Δ| = 0.04), contradicting Experiment 1's frozen-model refresh-necessity and confirming the persona lives in the weights. A mechanical contributor reinforces this: the full SCI (1338 tokens) overflows the model's 1024-token window, so conversation history — and any refresh — is dropped at inference regardless. The Experiment 6 agent is therefore *stateless per-turn*, which is itself why the 40-turn curve is flat and why long-horizon continuity requires an external memory layer (Experiment 7, Section 15.1).

15. **H2's residual gap to the aspirational 0.80 is a pretraining-objective limit, not scale.** At 321M the model is already BERT-large-sized; no-answer detection is a global-passage property natively suited to bidirectional MLM pretraining, whereas this trunk is causal-pretrained with only a short bidirectional retrofit. Keeping the trunk causal is deliberate — it is what lets the *same* model *be* ADA (generate the persona) rather than only read — so the ~0.78 abstention ceiling is the price of a single owned, generative trunk.

**Deployment.** The 321M `(corpus, weights)` artifact runs fully offline (int8, ~150–300 MB; ~50–100 tok/s on a Jetson Orin Nano, ~20–50 on a 2015-era laptop), realising the offline-resilience requirement of Section 5.9. Its deployment profile is specified in Section 17.6.

#### 15.2.6 Experiment 7: Long-Horizon Episodic Memory (extract-then-style)

Experiment 6's agent is *stateless per-turn* (§15.2.5, finding 14) — the 1338-token SCI overflows the 1024 window, so conversation history is always dropped. Experiment 7 tests whether an external **episodic-RAG** memory layer gives the owned agent genuine long-horizon recall over 100–500 turns. It is an inference-time experiment on the frozen Experiment 6 heads (LM head + span head), with a **compact SCI** (~421 tokens) freeing window space, all-MiniLM-L6-v2 retrieval, and three conditions: C0 no-memory, C1 sliding-window, C2 episodic-RAG.

**Key findings:**

16. **Generative memory-recall fails; extractive recall works — the role boundary, empirically forced.** Routing recall through the causal **LM head** gives out-of-window recall of only **0.20**: it fabricates values rather than reading them back from the retrieved memory. Memory-aware fine-tuning to fix this (two designs) is a **dead end** at 321M — it overfits and degrades the base reader. Routing recall through the discriminative **span head** (retrieve → span-extract → style), which can only point at a token span actually present in the retrieved text and so cannot fabricate, lifts out-of-window recall to **0.54** — and it is **lag-independent** (0.51 / 0.52 / 0.53 / 0.63 at lag 10 / 40 / 100 / 300) versus **0.00** on both baselines. Episodic factual recall belongs on the extractive head; dispositional persona on the generative head (Section 5.2).

17. **The read mechanism clears the bar; the end-to-end gap is write-side plant quality.** An oracle-store decomposition (ground-truth forced into memory) shows retrieval + span-extraction recalls at **0.81** with perfect storage — ≥ 0.70 at *every* lag including 300. The entire end-to-end gap (0.81 → 0.55) is **plant quality**: the 321M model writing imperfect answers into memory at store time. Retrieval is not the limit (hit-rate 1.00) and the read is not the limit (0.81) — the small model's *write* is. The lever is the write side: a larger-base reader/writer (the pre-registered fallback), not more retrieval or fine-tuning.

18. **Persona and cost extend cleanly.** PersonaScore holds at **3.79** over 100–500 turns (LM head untouched by the extractive path), and per-turn injected context is **O(1)** — prompt tokens 915 → 926 from low to high store (**1.01×**), latency flat — despite the store growing 5×. Persona and the memory scaffolding stay owned-and-small; only the peak-accuracy write path points to a larger reader.

19. **Baked fictional episodic memory is a deployment hazard, and re-grounds the E dimension.** Experiment 6's E score partly rested on *baking three fictional `salient_past_events` into the weights*. These are **false day-1 memories** — a freshly deployed ADA would "remember" sessions that never happened. Episodic content must instead live in the **dynamic Episodic Register** and be *read*, not baked (Section 17.3); the extractive path built here is that reading mechanism, motivating a **dynamic-memory re-grounding of the E dimension** (planned Experiment 8).

**Verdict.** Episodic-RAG + extract-then-style gives the owned agent genuine, lag-independent long-horizon recall at O(1) per-turn cost, with persona intact. The memory architecture is adopted; memory-aware fine-tuning is a documented dead end; the write-side plant-quality gap is closed by the larger-base fallback.
...
**Owned-small-model update (Experiment 7).** The partition above was derived for a *frozen 7B* relying on system-prompt content; at the owned-small-model scale (Section 15.2.6) two refinements hold. First, episodic content must be **dynamic and read, not baked into the weights**: Experiment 6 reached part of its E score by baking fixed *fictional* `salient_past_events` into the weights, which produces **false day-1 memories** — the agent recalls sessions that never occurred. Episodic facts must therefore live in the dynamic Episodic Register and be retrieved, not memorized. Second, episodic *recall* must run on the **discriminative reader, not the generative head**: routing recall through the generative transducer fabricates values (and memory-aware fine-tuning of it is a dead end at 321M), whereas an extractive span head over retrieved memory recalls reliably and lag-independently (retrieve → span-extract → style; recall ceiling 0.81, ≥ 0.70 at every lag). The Episodic Register is the source of episodic facts, retrieval + span-extraction is the read path, and the generative head styles the result in voice — the role boundary of Section 5.2 applied to memory. This motivates re-grounding the E evaluation on **dynamic** recall (planned Experiment 8) rather than recitation of baked fixtures.
...
**Offline-owned profile (Experiment 6).** A fourth profile departs from the shared-7B assumption of the table above: a from-scratch, fully-owned small model (Section 5.9, Section 15.2.5), validated at 321M for the daily-QA scenario at H3 = 3.80 across 40 turns. It runs fully offline (int8, ~150–300 MB) with the persona baked into the weights. For this profile the **SCI-refresh scheduler of the first three profiles is unnecessary**: Experiment 6's H4 result (R1 3.75 ≈ R0 3.80) shows a weight-baked persona holds across a conversation without turn-15/30 re-injection, so the refresh logic — and the assumption behind Section 17.3's episodic-partitioning constraint, which was derived for a *frozen* 7B relying on system-prompt content — can be omitted when the persona is compiled into the weights. This profile suits offline-resilient, single-owner deployments (edge devices, air-gapped installs) where the `(corpus, weights)` artifact must run and retrain without a network. Its limitation is context: at a 1024-token window the agent is stateless per-turn, so multi-session and long-horizon episodic continuity require an external session-memory layer (episodic-RAG), the subject of the planned Experiment 7.
...
Empirical validation of the self-modeling extension (Section 17) is now complete across Experiments 1, 2, and 6, with the QPM-interface ablations 3–5 established along the way (Section 5.9). Experiment 1 [65] established that 7B is the minimum viable SLM scale (3.8B incoherent on JSON-based SCI prompts) and that prompt-time architectural strategies cap mean PersonaScore at 3.20/5.0 — below the 3.5 pre-registered threshold, with the Combined SCI strategy producing a near-flat degradation profile (β≈0). Experiment 2 [67] resolved the residual gap with LoRA fine-tuning: the full Condition C architecture (Qwen2.5-7B + LoRA-10K SCI grounding adapter + Combined SCI) reaches mean PersonaScore 4.42/5.0, ΔE = +0.579 on the Episodic dimension, Cohen's d = 7.51 versus the no-LoRA control. The pre-registered Decision Rule (Section 15.4.2) triggered Outcome A, validating the full Phase 1 SMC architecture at 7B and retiring the previously planned 14B model test from the critical path. Experiment 6 then validated the owned-small-model direction (Section 5.9): a from-scratch, fully-owned **321M dual-head** model as ADA's daily-QA agent cleared every operational bar — grounded reading 0.77, calibrated abstention 0.78, persona **3.80/5.0** across 40 turns — with the persona and QPM **compiled into the weights**. This resolves the Experiment 3–5 interface-null (weight-level installation transmits the signal that every inference-time channel discarded), lets the 321M agent **beat a prompted 7B on every PersonaScore dimension** (3.80 vs 3.22) and reach parity with the fine-tuned 7B on the substantive dimensions, and shows a weight-baked persona needs **no SCI refresh** (Section 15.2.5) — all in a `(corpus, weights)` artifact that runs fully offline. Experiment 7 then extended the owned agent to a **long-running daily agent** (Section 15.2.6): an external episodic-RAG memory layer gives it genuine, lag-independent long-horizon recall at O(1) per-turn cost, with persona intact — provided recall runs on the **discriminative span head** (retrieve → span-extract → style), not the generative one, which fabricates (memory-aware fine-tuning of it is a dead end at this scale). The read mechanism clears the bar (recall ceiling 0.81); the residual end-to-end gap is the small model's write-side accuracy, closed by the larger-base fallback while persona and the O(1) memory scaffolding stay owned-and-small.

The post-hoc **H4 base-capability test** has now been completed and PASSED (+1.71% mean degradation on a 100-probe out-of-domain battery across 5 categories; paired t = −1.07, p = 0.287; Cohen's d_z = −0.107 — Section 15.4.2 and [67]). With H4 resolved, future work should prioritize: (a) **cross-model replication** on Llama 3.1 8B and Gemma 2 9B to promote Outcome A from a Qwen2.5-7B-specific finding to a 7B-class generalization; (b) **Episodic-stratified follow-up training** (~15K examples, ~50% E-dimension) to clear the Episodic threshold cheaper than the H5 curve's predicted ~20K under overall scaling; (c) a **wider H4 re-test focused on Instruction Following** (the one category that landed at the threshold with p = 0.0961 on n = 20) to confirm the result is a small-sample artifact rather than a real format-strict regression; (d) **long-horizon and multi-session persistence testing** beyond the 40-turn ceiling — the pre-registered Experiment 7 tests an episodic-RAG session-memory layer over 100–500-turn conversations, since the owned agent is stateless per-turn at its 1024-token window; (e) ablation studies comparing QPM-driven personality consistency against prompt-engineered LLM baselines; (f) field trials of domain-specialized agents in controlled therapeutic and educational settings; (g) physical prototype integration with an open humanoid platform; and (h) investigation of QPM parameter optimization via behavioral consistency feedback. A 14B + LoRA scale test remains scientifically informative as off-critical-path follow-up but is not required for the Phase 1 SMC deployment claim.
