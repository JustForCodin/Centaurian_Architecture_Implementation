...
4. **Does the SCI grounding adapter cause regression on out-of-domain capability** (coding, math, general knowledge)? **H4 deferred** — out-of-domain probe battery is the highest-priority remaining follow-up (Section 17.5). Open.
...
**Deferred follow-ups (not blocking Phase 1 deployment):** the H4 base-capability test (out-of-domain probe battery to verify the SCI grounding adapter does not cause catastrophic forgetting on coding, math, or general-knowledge tasks); cross-model replication on Llama 3.1 8B and Gemma 2 9B (to promote Outcome A from "Qwen-specific" to "7B-class generalization"); and Episodic-stratified follow-up training (predicted to clear the Episodic threshold of 3.5 with ~15K examples instead of the ~20K predicted by the H5 overall-scaling curve).
