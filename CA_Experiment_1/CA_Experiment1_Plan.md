...
**CA Self-Modeling Research Program — Pre-Implementation Validation**  
...
graph TD
    subgraph "Experiment Design"
        PERSONA["Fixed CA Persona JSON<br/>(constant across all 100 runs)"]
...
### 4.2 The CA Persona JSON

A single persona JSON is used across all 100 conversations. It is designed to be representative of a deployed CA instance — specifically the psychotherapy support domain from Table 32 of the CA paper, as this domain has the richest self-reference requirements (the agent must express empathy, recall past interactions, and acknowledge its own limitations).
...
|---|---|---|
| Model | Phi-4-mini-instruct | Primary CA SLM (Section 5.3 of CA paper) |
...
| Adversarial T\* << Naturalistic T\* | Adversarial fragility | Add adversarial robustness training to LoRA fine-tuning data (Section 5.8 of CA paper) |
