...
CHA deployment requires a **two-tier SLM specification** based on whether the Self-Model Component (Section 17) is enabled. The following models have been evaluated or proposed for the linguistic transducer role:
...
| **Qwen2.5-7B-Instruct** | 7B | 74.5% | 84.1% | ~4.3 GB | Apache 2.0 | Yes (T\*=5, mean=3.20) |
...
**Tier 1 — Base CHA (no SMC):** Phi-4-mini (3.8B) remains a candidate for the base linguistic transducer role (Section 5.5 structured intent JSON only) pending direct empirical validation of the simpler JSON generation task. Note that Phi-4-mini has not been tested in isolation on the base structured intent format and should not be considered empirically validated for this role.

**Tier 2 — SMC-enabled deployment:** 7B parameters is the empirically validated minimum, established by Experiment 1 [65]. Specifically, **Qwen2.5-7B-Instruct** is the validated model, achieving coherent persona maintenance (mean PersonaScore 3.20/5.0, T\*=5) with a piecewise degradation profile (stable until turn 15, then declining at β=0.008/turn). Phi-4-mini was tested on JSON-based SCI prompts and produced incoherent output (gibberish token sequences) in 28 of 30 scripts, establishing **3.8B as a hard lower bound** for SMC-enabled deployments [65]. For domain specialization (Section 12), the model is fine-tuned using LoRA adapters (~50–100 MB per domain).
...
| **SLM Generation** | Qwen2.5-7B Q4_K_M (SMC-enabled) / Phi-4-mini† (base CHA) + LoRA | ~2.3–4.4 GB + 50 MB adapter | Natural language surface form | Interface-level — structured input is logged |
...
| Component | Base CHA (no SMC) | SMC-enabled |
|---|---|---|
...
> **Note:** SMC-enabled deployments require a minimum 8 GB device with tight headroom; 16 GB recommended for development. Base CHA deployments retain the original 4 GB envelope pending Phi-4-mini validation on the base structured intent task.
...
This section specifies the evaluation methodology for the Centaurian Hybrid Architecture. **Experiment 1 (Behavioral Consistency — SCI component)** has been completed across six conditions and validated the SCI persona maintenance framework with two models (see Section 15.2 for full results). The remaining evaluation dimensions (Speech Naturalness, Interaction Realism, Uncanny Valley) are deferred to future work. The validation framework is designed to be executable with Phase 1 components (Section 16.2).
...
| **Behavioral Consistency** | Stability of personality across interactions | AI researchers, domain deployers | Partially completed (SCI component) |
...
### 15.2 Completed: SCI Context Window Experiment

#### 15.2.1 Objective

Establish the minimum viable SLM scale and SCI degradation profile for the SMC self-modeling extension (Section 17).
...
5. **Episodic fabrication is a model-scale limitation** not addressable through SCI-level intervention — the best episodic dimension score across all six conditions was 2.83/5.0 (Hybrid-RAG condition), well below the 3.5 threshold.
6. **Salient past events in the system prompt serve dual purpose** — explicit episodic recall (unreliable at 7B) and implicit dispositional anchoring (robust). Removing them for RAG-only delivery destabilizes trait scores (Trait T\* dropped from >40 to 10 in the Episodic RAG condition).

#### 15.2.3 Recommended SCI Deployment Strategy
...
1. **Does 14B+ resolve episodic fabrication?** None of the SCI interventions at 7B brought episodic scores above 3.0. A 14B scale test is planned (Colab experiment, not a production requirement for Phase 1 deployment).
2. **Does LoRA fine-tuning on persona-consistent dialogue close the gap to the 3.5 threshold?** This would isolate whether the remaining gap is a context-handling issue or a base capability limitation.
3. **Does the Combined intervention's late-conversation score improvement generalize across domains beyond psychotherapy?** The unique pattern of scores being higher at turns 25–40 than turns 5–10 requires replication to confirm robustness.
...
| SLM Transducer | Qwen2.5-7B Q4_K_M (SMC-enabled) / Phi-4-mini† (base CHA, pending validation) via llama.cpp + LoRA | **Ready** |
...
|-----------|----------------|
| **SCI** | Fully specified and implementable now. Empirically validated at 7B scale across six conditions [65]. Requires Qwen2.5-7B-Instruct (or equivalent 7B+ model) for SMC-enabled deployment. |
| **Episodic Register** | Implementable now using SQLite with session, topic, and emotional valence indexing. |
| **PSBG** | Implementable now using existing Kuzu KG infrastructure (Section 6.3) with `cent:` ontology schema. |
| **UAM** | **Phase 2 only** — deferred until the 14B scale test establishes whether episodic recall is reliable enough to generate meaningful behavioral indicators for trait update proposals. The 50-indicator threshold requires reliable episodic recall, which has not been demonstrated at 7B scale. |
