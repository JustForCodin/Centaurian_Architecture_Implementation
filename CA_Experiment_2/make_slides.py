#!/usr/bin/env python3
"""Generate the CA Experiments 1 & 2 presentation deck."""
...
EXP1 = ROOT / "CA_Experiment_1"
EXP2 = ROOT / "CA_Experiment_2"
OUT  = Path.home() / "Documents" / "CA_Experiments_1_2_Slides.pptx"
...
def footer(slide):
    ...
    p.text = "CA · Persona Consistency in Small LMs"
...
text(s, "The Centaurian Architecture",
...
bullets(s, [
    ...
    "Total memory footprint: ~4 GB (base CA) to ~6 GB (SMC-enabled with 7B SLM) — runs on edge hardware (Jetson Orin, Snapdragon 8 Elite, Apple Silicon)",
...
footer(s)
notes(s, "Three points. (1) The 'hybrid' in CA is symbolic/quantum reasoning plus neural transduction — NOT a small-model + large-model routing pattern. (2) The SLM does not reason; reasoning happens upstream in the QPM → BDI → KG pipeline. The SLM's job is to convert structured intent JSON into natural language. (3) Total memory footprint ~4–6 GB means the entire stack runs at the edge. There is no cloud-side large reasoning model.")
...
bullets(s, [
    ...
    "Tier 1 deployment (base CA): 1–4B SLM is sufficient (e.g., Phi-4-mini candidate)",
...
text(s, "Job 2 — Self-modeling (when SMC enabled)",
     ...
     size=13, bold=True, color=NAVY)
bullets(s, [
    "SMC = Self-Model Component (Section 17 of CA spec)",
...
text(s, "Why the SLM has to do this in-prompt (not via escalation)",
     ...
     size=13, bold=True, color=NAVY)
bullets(s, [
    "CA has no \"large reasoning model\" to escalate to — the entire stack runs at the edge in ~4–6 GB",
...
footer(s)
notes(s, "The key insight here: there's no fallback when the SLM fails to hold persona. CA is designed to run entirely at the edge in ~6 GB, so 'just use a bigger model when needed' isn't an option. Either the 7B SLM can hold the SCI reliably, or the SMC sub-architecture has to find another way to enforce consistency. That's exactly what Experiments 1 and 2 test.")
...
# ============================================================
s = new_slide()
title_bar(s, "The Central Question", "And what each possible answer means for CA's SMC deployment")
...
table(s,
      ...
      [
          (("YES", GREEN, True),
           ("SMC-enabled CA ships at 7B. Edge deployment (~6 GB total footprint) preserved. The SLM does both jobs (transduction + self-modeling) without escalation.", DARK)),
          (("NO", RED, True),
...
callout(s, "The whole program is built around a single deployment-relevant question: can we ship CA at 7B?",
...
text(s, "Phi-4-mini    ·    Qwen 7B baseline    ·    Best Exp 1 architecture    ·    LoRA-10K + Combined SCI",
     ...
     size=12, color=LIGHT, align=PP_ALIGN.CENTER)
text(s, "The CA SMC sub-architecture is ready for Phase 1 deployment at 7B.",
